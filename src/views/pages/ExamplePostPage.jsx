...
const ExamplePostPage = () => {
  // Variables required for the useAxiosQuery
  const m = { method: "post", url: "posts" };
  const postApi = useAxiosQuery(m);
  const {
    context,
    ...
    reset,
    status,
  } = postApi; // Use only the required variables

  console.log("postApi", postApi);

  // Action when the button is clicked
  const handleGetApiClick = () => {
    // Get a valid data before post-api call

    const data = {};
    postApi.mutate(data); // Actual api call
  };
