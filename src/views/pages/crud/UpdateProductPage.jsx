...
const UpdateProductPage = () => {
  ...
  // const updateProductApi = useAxiosQuery(m); // You can use below arguments if you want some modifiations as per the result from the (response)
  const updateProductApi = useAxiosQuery({ ...m, onSuccess, onError }); // You can use above variable if you dont want to add some modifications
  const {
    context,
    ...
    reset,
    status,
  } = updateProductApi; // Use only the required variables For eg:: const {isLoading, data: response } = updateProductApi
  ...
  const handleGetApiClick = () => {
    ...
    updateProductApi.mutate(data); // Actual api call
  };
  ...
  return (
    <div>
      ...
      >
        ...
        >
          Update Product with title (Click me)
        </button>
      </div>
      <hr />
      <hr />
      <div>
        <div>
          ...
          >
            data: {JSON.stringify(updatePostResponse ?? {})}
          </p>
        </div>
      </div>
    </div>
